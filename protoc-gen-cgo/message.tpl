...
{{if .Repeated}}//#include ".h" //ToDo: include correct .h file for corresponding C-struct of "Repeated" field{{end}}import "C"
import (
    "fmt"
...
{{.FieldName}}: make([]*{{.ProtoFileName}}.{{.DataType}}, 0),//ToDo: Check if protobuf structure is implemented correctly (mainly naming){{end}}
...
switch choice := {{lowCaseFirstLetter .MessageName}}.{{.MessageName}}.(type) {
...
switch {{lowCaseFirstLetter .MessageName}}C.present {
{{ range $fieldIndex, $field := $fields }}
