...
func PerEncodeE2SmRcPreControlOutcome(co *e2smrcprev2.E2SmRcPreControlOutcome) ([]byte, error) {
	...
	per, err := aper.MarshalWithParams(co, "choiceExt", e2smrcprev2.RcPreChoicemap, nil)
	if err != nil {
		return nil, err
	}
...
func PerDecodeE2SmRcPreControlOutcome(per []byte) (*e2smrcprev2.E2SmRcPreControlOutcome, error) {

	log.Debugf("Obtained E2SM-RC-PRE-ControlOutcome PER bytes are\n%v", hex.Dump(per))

	result := e2smrcprev2.E2SmRcPreControlOutcome{}
