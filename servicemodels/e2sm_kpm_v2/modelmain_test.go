...
func TestServicemodel_IndicationMessageProtoToASN1(t *testing.T) {
	...
	newE2SmKpmPdu, err := pdubuilder.CreateE2SmKpmIndicationMessageFormat1(subscriptionID, cellObjID, &measInfoList, &measData)
	...
	assert.Equal(t, 147, len(protoBytes))
...
func TestServicemodel_Asn1BytesToByte(t *testing.T) {
	indHdrFmt1Hex, err := kpmv2TestSm.Asn1BytesToByte("1f21222324187478740000034f4e4640736f6d6554797065066f6e660c3734370" +
		"0d4bc08803039201a85")
	assert.NilError(t, err)
	fmt.Printf("Output of Asn1BytesToByte is \n%x\n", indHdrFmt1Hex)

	inHdrBytes, err := kpmv2TestSm.IndicationHeaderASN1toProto(indHdrFmt1Hex)
	assert.NilError(t, err, "unexpected error converting asn1Bytes to protoBytes")
	assert.Assert(t, inHdrBytes != nil)
	assert.Equal(t, 68, len(inHdrBytes))
	testIH := &e2sm_kpm_v2.E2SmKpmIndicationHeader{}
	err = proto.Unmarshal(inHdrBytes, testIH)
	assert.NilError(t, err)
	t.Logf("Decoded message (IndicationHeaderFormat1) is \n%v\n", testIH)

	indMsgFmt1Hex, err := kpmv2TestSm.Asn1BytesToByte("0e4030380000036f6e66011500004020747269616c013ffba021222340400" +
		"10203000a7c0f000f0001724000fa00000400007a0001c7000314000000400300023039440480001a8500")
	assert.NilError(t, err)
	fmt.Printf("Output of Asn1BytesToByte is \n%x\n", indMsgFmt1Hex)

	indMsgFmt1Bytes, err := kpmv2TestSm.IndicationMessageASN1toProto(indMsgFmt1Hex)
	assert.NilError(t, err, "unexpected error converting protoBytes to asn1Bytes")
	assert.Assert(t, indMsgFmt1Bytes != nil)
	assert.Equal(t, 123, len(indMsgFmt1Bytes))
	testIM := &e2sm_kpm_v2.E2SmKpmIndicationMessage{}
	err = proto.Unmarshal(indMsgFmt1Bytes, testIM)
	assert.NilError(t, err)
	t.Logf("Decoded message (IndicationMessageFormat1) is \n%v", testIM)

	actDefFmt3Hex, err := kpmv2TestSm.Asn1BytesToByte("00010c4000036f6e6600000040747269616c000048210200c90115203038")
	assert.NilError(t, err)
	fmt.Printf("Output of Asn1BytesToByte is \n%x\n", actDefFmt3Hex)

	actDefFmt3Bytes, err := kpmv2TestSm.ActionDefinitionASN1toProto(actDefFmt3Hex)
	assert.NilError(t, err, "unexpected error converting protoBytes to asn1Bytes")
	assert.Assert(t, actDefFmt3Bytes != nil)
	assert.Equal(t, 54, len(actDefFmt3Bytes))
	testAD := &e2sm_kpm_v2.E2SmKpmActionDefinition{}
	err = proto.Unmarshal(actDefFmt3Bytes, testAD)
	assert.NilError(t, err)
	t.Logf("Decoded message (ActionDefinitionFormat3) is \n%v\n", testAD)

	evntTrigDefHex, err := kpmv2TestSm.Asn1BytesToByte("00010c4000036f6e6600000040747269616c000048210200c90115203038")
	assert.NilError(t, err)
	fmt.Printf("Output of Asn1BytesToByte is \n%x\n", evntTrigDefHex)

	evntTrigDefBytes, err := kpmv2TestSm.EventTriggerDefinitionASN1toProto(evntTrigDefHex)
	assert.NilError(t, err, "unexpected error converting protoBytes to asn1Bytes")
	assert.Assert(t, evntTrigDefBytes != nil)
	assert.Equal(t, 4, len(evntTrigDefBytes))
	testETD := &e2sm_kpm_v2.E2SmKpmEventTriggerDefinition{}
	err = proto.Unmarshal(evntTrigDefBytes, testETD)
	assert.NilError(t, err)
	t.Logf("Decoded message (EventTriggerDefinition) is \n%v\n", testETD)

	ranFuncDescHex, err := kpmv2TestSm.Asn1BytesToByte("74046f6e660000056f69643132330700736f6d654465736372697074696f6e" +
	...
	ranFuncDescBytes, err := kpmv2TestSm.RanFuncDescriptionASN1toProto(ranFuncDescHex)
	assert.NilError(t, err, "unexpected error converting protoBytes to asn1Bytes")
	assert.Assert(t, ranFuncDescBytes != nil)
	assert.Equal(t, 169, len(ranFuncDescBytes))
	testRFD := &e2sm_kpm_v2.E2SmKpmRanfunctionDescription{}
...
func TestServicemodel_HexDumpToByte(t *testing.T) {
	indHdrFmt1Hex, err := kpmv2TestSm.HexDumpToByte("00000000  1f 21 22 23 24 18 74 78  74 00 00 03 4f 4e 46 40  " +
	...
	indHdrFmt1Bytes, err := kpmv2TestSm.IndicationHeaderASN1toProto(indHdrFmt1Hex)
	assert.NilError(t, err, "unexpected error converting asn1Bytes to protoBytes")
	assert.Assert(t, indHdrFmt1Bytes != nil)
	assert.Equal(t, 68, len(indHdrFmt1Bytes))
	testIH := &e2sm_kpm_v2.E2SmKpmIndicationHeader{}
	err = proto.Unmarshal(indHdrFmt1Bytes, testIH)
	assert.NilError(t, err)
	t.Logf("Decoded message (IndicationHeaderFormat1) is \n%v", testIH)

	indMsgFmt1Hex, err := kpmv2TestSm.HexDumpToByte("00000000  0e 40 30 38 00 00 03 6f  6e 66 01 15 00 00 40 20  |.@08...onf....@ |" +
	...
	actDefFmt3Hex, err := kpmv2TestSm.HexDumpToByte("00000000  00 01 0c 40 00 03 6f 6e  66 00 00 00 40 74 72 69  " +
	...
	evntTrigDefHex, err := kpmv2TestSm.HexDumpToByte("00000000  00 01 0c                                          |...|")
	assert.NilError(t, err)
	fmt.Printf("Output of HexDumpToByte is \n%x\n", evntTrigDefHex)
	...
	ranFuncDescHex, err := kpmv2TestSm.HexDumpToByte("00000000  74 04 6f 6e 66 00 00 05  6f 69 64 31 32 33 07 00  |t.onf...oid123..|" +
