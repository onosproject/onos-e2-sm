...
func xerEncodeMeasurementLabel(measurementLabel *e2sm_kpm_v2.MeasurementLabel) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_MeasurementLabel_KPMv2, unsafe.Pointer(measurementLabelCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeMeasurementLabel() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeMeasurementLabel(measurementLabel *e2sm_kpm_v2.MeasurementLabel) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_MeasurementLabel_KPMv2, unsafe.Pointer(measurementLabelCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeMeasurementLabel() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeMeasurementLabel(bytes []byte) (*e2sm_kpm_v2.MeasurementLabel, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_MeasurementLabel_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeMeasurementLabel((*C.MeasurementLabel_KPMv2_t)(unsafePtr))
}

func perDecodeMeasurementLabel(bytes []byte) (*e2sm_kpm_v2.MeasurementLabel, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_MeasurementLabel_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newMeasurementLabel(measurementLabel *e2sm_kpm_v2.MeasurementLabel) (*C.MeasurementLabel_KPMv2_t, error) {

	measurementLabelC := C.MeasurementLabel_KPMv2_t{}
	var err error
	...
	if measurementLabel.SUm != -1 {
		var sUmC C.MeasurementLabel_KPMv2__sUM_t
		switch measurementLabel.SUm {
		case e2sm_kpm_v2.SUM_SUM_TRUE:
			sUmC = C.MeasurementLabel_KPMv2__sUM_true
		default:
	...
	if measurementLabel.PreLabelOverride != -1 {
		var preLabelOverrideC C.MeasurementLabel_KPMv2__preLabelOverride_t
		switch measurementLabel.PreLabelOverride {
		case e2sm_kpm_v2.PreLabelOverride_PRE_LABEL_OVERRIDE_TRUE:
			preLabelOverrideC = C.MeasurementLabel_KPMv2__preLabelOverride_true
		default:
	...
	if measurementLabel.StartEndInd != -1 {
		var startEndIndC C.MeasurementLabel_KPMv2__startEndInd_t
		switch measurementLabel.StartEndInd {
		case e2sm_kpm_v2.StartEndInd_START_END_IND_START:
			startEndIndC = C.MeasurementLabel_KPMv2__startEndInd_start
		case e2sm_kpm_v2.StartEndInd_START_END_IND_END:
			startEndIndC = C.MeasurementLabel_KPMv2__startEndInd_end
		default:
...
func decodeMeasurementLabel(measurementLabelC *C.MeasurementLabel_KPMv2_t) (*e2sm_kpm_v2.MeasurementLabel, error) {
...
func decodeMeasurementLabelBytes(array [8]byte) (*e2sm_kpm_v2.MeasurementLabel, error) {
	mlC := (*C.MeasurementLabel_KPMv2_t)(unsafe.Pointer(uintptr(binary.LittleEndian.Uint64(array[0:]))))
