...
func xerEncodeMeasurementData(measurementData *e2sm_kpm_v2.MeasurementData) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_MeasurementData_KPMv2, unsafe.Pointer(measurementDataCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeMeasurementData() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeMeasurementData(measurementData *e2sm_kpm_v2.MeasurementData) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_MeasurementData_KPMv2, unsafe.Pointer(measurementDataCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeMeasurementData() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeMeasurementData(bytes []byte) (*e2sm_kpm_v2.MeasurementData, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_MeasurementData_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeMeasurementData((*C.MeasurementData_KPMv2_t)(unsafePtr))
}

func perDecodeMeasurementData(bytes []byte) (*e2sm_kpm_v2.MeasurementData, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_MeasurementData_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newMeasurementData(measurementData *e2sm_kpm_v2.MeasurementData) (*C.MeasurementData_KPMv2_t, error) {

	measurementDataC := new(C.MeasurementData_KPMv2_t)
...
func decodeMeasurementData(measurementDataC *C.MeasurementData_KPMv2_t) (*e2sm_kpm_v2.MeasurementData, error) {
	...
	for i := 0; i < ieCount; i++ {
		...
		ieC := *(**C.MeasurementDataItem_KPMv2_t)(unsafe.Pointer(uintptr(unsafe.Pointer(measurementDataC.list.array)) + offset))
