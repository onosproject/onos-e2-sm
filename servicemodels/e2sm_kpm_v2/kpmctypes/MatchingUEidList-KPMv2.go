...
func xerEncodeMatchingUeIDList(matchingUeIDList *e2sm_kpm_v2.MatchingUeidList) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_MatchingUEidList_KPMv2, unsafe.Pointer(matchingUeIDListCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeMatchingUeIDList() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeMatchingUeIDList(matchingUeIDList *e2sm_kpm_v2.MatchingUeidList) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_MatchingUEidList_KPMv2, unsafe.Pointer(matchingUeIDListCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeMatchingUeIDList() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeMatchingUeIDList(bytes []byte) (*e2sm_kpm_v2.MatchingUeidList, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_MatchingUEidList_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeMatchingUeIDList((*C.MatchingUEidList_KPMv2_t)(unsafePtr))
}

func perDecodeMatchingUeIDList(bytes []byte) (*e2sm_kpm_v2.MatchingUeidList, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_MatchingUEidList_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newMatchingUeIDList(matchingUeIDList *e2sm_kpm_v2.MatchingUeidList) (*C.MatchingUEidList_KPMv2_t, error) {

	matchingUeIDListC := new(C.MatchingUEidList_KPMv2_t)
...
func decodeMatchingUeIDList(matchingUeIDListC *C.MatchingUEidList_KPMv2_t) (*e2sm_kpm_v2.MatchingUeidList, error) {
	...
	for i := 0; i < ieCount; i++ {
		...
		ieC := *(**C.MatchingUEidItem_KPMv2_t)(unsafe.Pointer(uintptr(unsafe.Pointer(matchingUeIDListC.list.array)) + offset))
