...
func xerEncodeCellGlobalID(cellGlobalID *e2sm_kpm_v2.CellGlobalId) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_CellGlobalID_KPMv2, unsafe.Pointer(cellGlobalIDCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeCellGlobalID() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeCellGlobalID(cellGlobalID *e2sm_kpm_v2.CellGlobalId) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_CellGlobalID_KPMv2, unsafe.Pointer(cellGlobalIDCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeCellGlobalID() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeCellGlobalID(bytes []byte) (*e2sm_kpm_v2.CellGlobalId, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_CellGlobalID_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeCellGlobalID((*C.CellGlobalID_KPMv2_t)(unsafePtr))
}

func perDecodeCellGlobalID(bytes []byte) (*e2sm_kpm_v2.CellGlobalId, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_CellGlobalID_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newCellGlobalID(cellGlobalID *e2sm_kpm_v2.CellGlobalId) (*C.CellGlobalID_KPMv2_t, error) {

	var pr C.CellGlobalID_KPMv2_PR
	choiceC := [8]byte{}

	switch choice := cellGlobalID.CellGlobalId.(type) {
	case *e2sm_kpm_v2.CellGlobalId_NrCgi:
		pr = C.CellGlobalID_KPMv2_PR_nr_CGI
	...
	case *e2sm_kpm_v2.CellGlobalId_EUtraCgi:
		pr = C.CellGlobalID_KPMv2_PR_eUTRA_CGI
	...
	cellGlobalIDC := C.CellGlobalID_KPMv2_t{
		present: pr,
		choice:  choiceC,
	}
...
func decodeCellGlobalID(cellGlobalIDC *C.CellGlobalID_KPMv2_t) (*e2sm_kpm_v2.CellGlobalId, error) {

	//This is Decoder part (OneOf)
	cellGlobalID := new(e2sm_kpm_v2.CellGlobalId)

	switch cellGlobalIDC.present {
	case C.CellGlobalID_KPMv2_PR_nr_CGI:
	...
	case C.CellGlobalID_KPMv2_PR_eUTRA_CGI:
	...
	case C.CellGlobalID_KPMv2_PR_NOTHING:
