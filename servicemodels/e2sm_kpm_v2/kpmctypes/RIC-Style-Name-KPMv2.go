...
func xerEncodeRicStyleName(ricStyleName *e2sm_kpm_v2.RicStyleName) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_RIC_Style_Name_KPMv2, unsafe.Pointer(ricStyleNameCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeRicStyleName() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeRicStyleName(ricStyleName *e2sm_kpm_v2.RicStyleName) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_RIC_Style_Name_KPMv2, unsafe.Pointer(ricStyleNameCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeRicStyleName() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeRicStyleName(bytes []byte) (*e2sm_kpm_v2.RicStyleName, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_RIC_Style_Name_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeRicStyleName((*C.RIC_Style_Name_KPMv2_t)(unsafePtr))
}

func perDecodeRicStyleName(bytes []byte) (*e2sm_kpm_v2.RicStyleName, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_RIC_Style_Name_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newRicStyleName(ricStyleName *e2sm_kpm_v2.RicStyleName) (*C.RIC_Style_Name_KPMv2_t, error) {
...
func decodeRicStyleName(ricStyleNameC *C.RIC_Style_Name_KPMv2_t) (*e2sm_kpm_v2.RicStyleName, error) {
