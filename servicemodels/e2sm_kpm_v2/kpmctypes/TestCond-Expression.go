...
func decodeTestCondExpressionBytes(array [8]byte) (*e2sm_kpm_v2.TestCondExpression, error) {
	testCondExpressionC := (*C.TestCond_Expression_t)(unsafe.Pointer(uintptr(binary.LittleEndian.Uint64(array[0:]))))

	return decodeTestCondExpression(testCondExpressionC)
}
