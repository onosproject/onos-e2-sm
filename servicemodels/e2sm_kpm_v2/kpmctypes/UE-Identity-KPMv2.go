...
func xerEncodeUeIdentity(ueIdentity *e2sm_kpm_v2.UeIdentity) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_UE_Identity_KPMv2, unsafe.Pointer(ueIdentityCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeUeIdentity() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeUeIdentity(ueIdentity *e2sm_kpm_v2.UeIdentity) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_UE_Identity_KPMv2, unsafe.Pointer(ueIdentityCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeUeIdentity() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeUeIdentity(bytes []byte) (*e2sm_kpm_v2.UeIdentity, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_UE_Identity_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeUeIdentity((*C.UE_Identity_KPMv2_t)(unsafePtr))
}

func perDecodeUeIdentity(bytes []byte) (*e2sm_kpm_v2.UeIdentity, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_UE_Identity_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newUeIdentity(ueIdentity *e2sm_kpm_v2.UeIdentity) (*C.UE_Identity_KPMv2_t, error) {
...
func decodeUeIdentity(ueIdentityC *C.UE_Identity_KPMv2_t) (*e2sm_kpm_v2.UeIdentity, error) {
