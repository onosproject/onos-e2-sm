...
//#include <stdlib.h>
//#include <assert.h>
//#include "MeasurementDataItem-KPMv2.h"
import "C"
...
func xerEncodeMeasurementDataItem(measurementDataItem *e2sm_kpm_v2.MeasurementDataItem) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_MeasurementDataItem_KPMv2, unsafe.Pointer(measurementDataItemCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeMeasurementDataItem() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeMeasurementDataItem(measurementDataItem *e2sm_kpm_v2.MeasurementDataItem) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_MeasurementDataItem_KPMv2, unsafe.Pointer(measurementDataItemCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeMeasurementDataItem() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeMeasurementDataItem(bytes []byte) (*e2sm_kpm_v2.MeasurementDataItem, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_MeasurementDataItem_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeMeasurementDataItem((*C.MeasurementDataItem_KPMv2_t)(unsafePtr))
}

func perDecodeMeasurementDataItem(bytes []byte) (*e2sm_kpm_v2.MeasurementDataItem, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_MeasurementDataItem_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newMeasurementDataItem(measurementDataItem *e2sm_kpm_v2.MeasurementDataItem) (*C.MeasurementDataItem_KPMv2_t, error) {
...
func decodeMeasurementDataItem(measurementDataItemC *C.MeasurementDataItem_KPMv2_t) (*e2sm_kpm_v2.MeasurementDataItem, error) {
