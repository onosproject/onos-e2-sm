...
func xerEncodeMatchingCondItem(matchingCondItem *e2sm_kpm_v2.MatchingCondItem) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_MatchingCondItem_KPMv2, unsafe.Pointer(matchingCondItemCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeMatchingCondItem() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeMatchingCondItem(matchingCondItem *e2sm_kpm_v2.MatchingCondItem) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_MatchingCondItem_KPMv2, unsafe.Pointer(matchingCondItemCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeMatchingCondItem() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeMatchingCondItem(bytes []byte) (*e2sm_kpm_v2.MatchingCondItem, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_MatchingCondItem_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeMatchingCondItem((*C.MatchingCondItem_KPMv2_t)(unsafePtr))
}

func perDecodeMatchingCondItem(bytes []byte) (*e2sm_kpm_v2.MatchingCondItem, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_MatchingCondItem_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newMatchingCondItem(matchingCondItem *e2sm_kpm_v2.MatchingCondItem) (*C.MatchingCondItem_KPMv2_t, error) {

	var pr C.MatchingCondItem_KPMv2_PR
	choiceC := [8]byte{}

	switch choice := matchingCondItem.MatchingCondItem.(type) {
	case *e2sm_kpm_v2.MatchingCondItem_MeasLabel:
		pr = C.MatchingCondItem_KPMv2_PR_measLabel
	...
	case *e2sm_kpm_v2.MatchingCondItem_TestCondInfo:
		pr = C.MatchingCondItem_KPMv2_PR_testCondInfo
	...
	matchingCondItemC := C.MatchingCondItem_KPMv2_t{
		present: pr,
		choice:  choiceC,
	}
...
func decodeMatchingCondItem(matchingCondItemC *C.MatchingCondItem_KPMv2_t) (*e2sm_kpm_v2.MatchingCondItem, error) {

	matchingCondItem := new(e2sm_kpm_v2.MatchingCondItem)

	switch matchingCondItemC.present {
	case C.MatchingCondItem_KPMv2_PR_measLabel:
	...
	case C.MatchingCondItem_KPMv2_PR_testCondInfo:
	...
	case C.MatchingCondItem_KPMv2_PR_NOTHING:
