...
func xerEncodeMeasurementType(measurementType *e2sm_kpm_v2.MeasurementType) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_MeasurementType_KPMv2, unsafe.Pointer(measurementTypeCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeMeasurementType() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeMeasurementType(measurementType *e2sm_kpm_v2.MeasurementType) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_MeasurementType_KPMv2, unsafe.Pointer(measurementTypeCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeMeasurementType() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeMeasurementType(bytes []byte) (*e2sm_kpm_v2.MeasurementType, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_MeasurementType_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeMeasurementType((*C.MeasurementType_KPMv2_t)(unsafePtr))
}

func perDecodeMeasurementType(bytes []byte) (*e2sm_kpm_v2.MeasurementType, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_MeasurementType_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newMeasurementType(measurementType *e2sm_kpm_v2.MeasurementType) (*C.MeasurementType_KPMv2_t, error) {

	var pr C.MeasurementType_KPMv2_PR
	choiceC := [40]byte{}

	switch choice := measurementType.GetMeasurementType().(type) {
	case *e2sm_kpm_v2.MeasurementType_MeasName:
		pr = C.MeasurementType_KPMv2_PR_measName
	...
	case *e2sm_kpm_v2.MeasurementType_MeasId:
		pr = C.MeasurementType_KPMv2_PR_measID
	...
	measurementTypeC := C.MeasurementType_KPMv2_t{
		present: pr,
		choice:  choiceC,
	}
...
func decodeMeasurementType(measurementTypeC *C.MeasurementType_KPMv2_t) (*e2sm_kpm_v2.MeasurementType, error) {

	measurementType := new(e2sm_kpm_v2.MeasurementType)

	switch measurementTypeC.present {
	case C.MeasurementType_KPMv2_PR_measName:
		var a [16]byte
	...
	case C.MeasurementType_KPMv2_PR_measID:
		var a [8]byte
		copy(a[:], measurementTypeC.choice[0:])
	...
	case C.MeasurementType_KPMv2_PR_NOTHING:
