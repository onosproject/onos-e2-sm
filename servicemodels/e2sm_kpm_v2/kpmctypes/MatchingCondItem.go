...
func decodeMatchingCondItemBytes(array [8]byte) (*e2sm_kpm_v2.MatchingCondItem, error) {
	matchingCondItemC := (*C.MatchingCondItem_t)(unsafe.Pointer(uintptr(binary.LittleEndian.Uint64(array[0:8]))))

	return decodeMatchingCondItem(matchingCondItemC)
}
