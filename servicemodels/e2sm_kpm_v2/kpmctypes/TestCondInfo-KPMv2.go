...
func xerEncodeTestCondInfo(testCondInfo *e2sm_kpm_v2.TestCondInfo) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_TestCondInfo_KPMv2, unsafe.Pointer(testCondInfoCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeTestCondInfo() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeTestCondInfo(testCondInfo *e2sm_kpm_v2.TestCondInfo) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_TestCondInfo_KPMv2, unsafe.Pointer(testCondInfoCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeTestCondInfo() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeTestCondInfo(bytes []byte) (*e2sm_kpm_v2.TestCondInfo, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_TestCondInfo_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeTestCondInfo((*C.TestCondInfo_KPMv2_t)(unsafePtr))
}

func perDecodeTestCondInfo(bytes []byte) (*e2sm_kpm_v2.TestCondInfo, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_TestCondInfo_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newTestCondInfo(testCondInfo *e2sm_kpm_v2.TestCondInfo) (*C.TestCondInfo_KPMv2_t, error) {
	...
	testCondInfoC := C.TestCondInfo_KPMv2_t{
		testType:  *testTypeC,
		testExpr:  *testExprC,
...
func decodeTestCondInfo(testCondInfoC *C.TestCondInfo_KPMv2_t) (*e2sm_kpm_v2.TestCondInfo, error) {
...
func decodeTestCondInfoBytes(array [8]byte) (*e2sm_kpm_v2.TestCondInfo, error) {
	testCondInfoC := (*C.TestCondInfo_KPMv2_t)(unsafe.Pointer(uintptr(binary.LittleEndian.Uint64(array[0:8]))))
