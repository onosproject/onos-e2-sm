...
func xerEncodeRicKpmnodeItem(ricKpmnodeItem *e2sm_kpm_v2.RicKpmnodeItem) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_RIC_KPMNode_Item_KPMv2, unsafe.Pointer(ricKpmnodeItemCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeRicKpmnodeItem() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeRicKpmnodeItem(ricKpmnodeItem *e2sm_kpm_v2.RicKpmnodeItem) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_RIC_KPMNode_Item_KPMv2, unsafe.Pointer(ricKpmnodeItemCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeRicKpmnodeItem() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeRicKpmnodeItem(bytes []byte) (*e2sm_kpm_v2.RicKpmnodeItem, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_RIC_KPMNode_Item_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeRicKpmnodeItem((*C.RIC_KPMNode_Item_KPMv2_t)(unsafePtr))
}

func perDecodeRicKpmnodeItem(bytes []byte) (*e2sm_kpm_v2.RicKpmnodeItem, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_RIC_KPMNode_Item_KPMv2)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newRicKpmnodeItem(ricKpmnodeItem *e2sm_kpm_v2.RicKpmnodeItem) (*C.RIC_KPMNode_Item_KPMv2_t, error) {
	...
	if ricKpmnodeItem.CellMeasurementObjectList != nil {
		cellMeasurementObjectListC := new(C.struct_RIC_KPMNode_Item_KPMv2__cell_Measurement_Object_List)
...
func decodeRicKpmnodeItem(ricKpmnodeItemC *C.RIC_KPMNode_Item_KPMv2_t) (*e2sm_kpm_v2.RicKpmnodeItem, error) {
	...
	if ricKpmnodeItemC.cell_Measurement_Object_List != nil {
		...
		for i := 0; i < ieCount; i++ {
			...
			ieC := *(**C.Cell_Measurement_Object_Item_KPMv2_t)(unsafe.Pointer(uintptr(unsafe.Pointer(ricKpmnodeItemC.cell_Measurement_Object_List.list.array)) + offset))
