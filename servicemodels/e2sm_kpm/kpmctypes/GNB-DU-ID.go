...
func xerEncodeGnbDuId(gnbDuId *e2sm_kpm_ies.GnbDuId) ([]byte, error) {
	gnbDuIdC := newGnbDuId(gnbDuId)

	bytes, err := encodeXer(&C.asn_DEF_GNB_DU_ID, unsafe.Pointer(gnbDuIdC))
	if err != nil {
		return nil, err
	}
	return bytes, nil
...
func decodeGnbDuId(gnbDuIdC *C.GNB_DU_ID_t) (*e2sm_kpm_ies.GnbDuId, error) {
	...
	if err == nil {
		return nil, fmt.Errorf("decodeGnbDuId error in str-to-int convertion %T", err)
	}
