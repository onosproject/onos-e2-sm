...
type GlobalKpmNodeType int32
//type GNbCuUpId int64
//type GNbDuId int64

const (
	KpmNodeTypeGnbId GlobalKpmNodeType = iota
	KpmNodeTypeEnGnbId
	KpmNodeTypeNgEnbId
	KpmNodeTypeEnbId
)

//ToDo: maybe make a separate type for GnbId where GnbCuUpId and GnbDuId would be included?

// E2NodeIdentity a simplified model of the E2 Node
type GlobalKpmNodeIdentity struct {
	Plmn           PlmnID
	NodeType       GlobalKpmNodeType
	NodeIdentifier []byte
}

func NewKpmNodeIdentity(plmnIDSlice []byte) (*GlobalKpmNodeIdentity, error) {
	plmnID, err := PlmnIDFromSlice(plmnIDSlice)
	if err != nil {
		return nil, err
	}

	return &GlobalKpmNodeIdentity{
		Plmn: plmnID,
	}, nil
}
