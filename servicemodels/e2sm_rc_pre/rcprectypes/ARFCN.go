...
func xerEncodeARFCN(arfcn *e2sm_rc_pre_v2.Arfcn) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_ARFCN, unsafe.Pointer(arfcnCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeArfcn() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeARFCN(arfcn *e2sm_rc_pre_v2.Arfcn) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_ARFCN, unsafe.Pointer(arfcnCP))
	if err != nil {
...
func xerDecodeARFCN(bytes []byte) (*e2sm_rc_pre_v2.Arfcn, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_ARFCN)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeARFCN((*C.ARFCN_t)(unsafePtr))
}

func perDecodeARFCN(bytes []byte) (*e2sm_rc_pre_v2.Arfcn, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_ARFCN)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
