...
func newNRCellIdentity(nrCellIdentity *e2sm_rc_pre_v2.NrcellIdentity) *C.NRCellIdentity_t {
	nrCellIdentityC := newBitString(nrCellIdentity.Value)

	return nrCellIdentityC
}

func decodeNRCellIdentity(nrCellIdentityC *C.NRCellIdentity_t) (*e2sm_rc_pre_v2.NrcellIdentity, error) {
