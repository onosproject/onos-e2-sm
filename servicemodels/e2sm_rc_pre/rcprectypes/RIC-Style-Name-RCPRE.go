...
func xerEncodeRicStyleName(ricStyleName *e2sm_rc_pre_v2.RicStyleName) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_RIC_Style_Name_RCPRE, unsafe.Pointer(ricStyleNameCP))
	if err != nil {
...
func perEncodeRicStyleName(ricStyleName *e2sm_rc_pre_v2.RicStyleName) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_RIC_Style_Name_RCPRE, unsafe.Pointer(ricStyleNameCP))
	if err != nil {
...
func xerDecodeRicStyleName(bytes []byte) (*e2sm_rc_pre_v2.RicStyleName, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_RIC_Style_Name_RCPRE)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeRicStyleName((*C.RIC_Style_Name_RCPRE_t)(unsafePtr)), nil
}

func perDecodeRicStyleName(bytes []byte) (*e2sm_rc_pre_v2.RicStyleName, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_RIC_Style_Name_RCPRE)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newRicStyleName(ricStyleName *e2sm_rc_pre_v2.RicStyleName) *C.RIC_Style_Name_RCPRE_t {
	ricStyleNameCP := newPrintableString(ricStyleName.Value)

	return ricStyleNameCP
}

func decodeRicStyleName(ricStyleNameC *C.RIC_Style_Name_RCPRE_t) *e2sm_rc_pre_v2.RicStyleName {
