...
func xerEncodeEUTRACGI(eutracgi *e2sm_rc_pre_v2.Eutracgi) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_EUTRACGI_RCPRE, unsafe.Pointer(eutracgiCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeEUTRACGI() %s", err.Error())
	}
	return bytes, nil
...
func decodeEUTRACGI(eutracgiC *C.EUTRACGI_RCPRE_t) (*e2sm_rc_pre_v2.Eutracgi, error) {
...
func decodeEUTRACGIBytes(array [8]byte) *e2sm_rc_pre_v2.Eutracgi {
	eutracgiC := (*C.EUTRACGI_RCPRE_t)(unsafe.Pointer(uintptr(binary.LittleEndian.Uint64(array[0:8]))))
