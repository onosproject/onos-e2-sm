...
func xerEncodeNRT(nrt *e2sm_rc_pre_v2.Nrt) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_NRT_RCPRE, unsafe.Pointer(nrtCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeNRT() %s", err.Error())
	}
	return bytes, nil
...
func newNRT(nrt *e2sm_rc_pre_v2.Nrt) (*C.NRT_RCPRE_t, error) {
...
func decodeNRT(nrtC *C.NRT_RCPRE_t) (*e2sm_rc_pre_v2.Nrt, error) {
