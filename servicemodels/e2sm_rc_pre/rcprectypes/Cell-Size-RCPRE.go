...
func xerEncodeCellSize(cellSize e2sm_rc_pre_v2.CellSize) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_Cell_Size_RCPRE, unsafe.Pointer(&cellSizeC))
	if err != nil {
...
func perEncodeCellSize(cellSize e2sm_rc_pre_v2.CellSize) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_Cell_Size_RCPRE, unsafe.Pointer(&cellSizeC))
	if err != nil {
...
func xerDecodeCellSize(bytes []byte) (e2sm_rc_pre_v2.CellSize, error) {
	...
	return decodeCellSize((*C.Cell_Size_RCPRE_t)(unsafePtr)), nil
}

func perDecodeCellSize(bytes []byte) (e2sm_rc_pre_v2.CellSize, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_Cell_Size_RCPRE)
	if err != nil {
		return 0, err
	}
	if unsafePtr == nil {
...
func newCellSize(cellSize e2sm_rc_pre_v2.CellSize) (C.Cell_Size_RCPRE_t, error) {
	var ret C.Cell_Size_RCPRE_t
	switch cellSize {
	case e2sm_rc_pre_v2.CellSize_CELL_SIZE_FEMTO:
		ret = C.Cell_Size_RCPRE_femto
	case e2sm_rc_pre_v2.CellSize_CELL_SIZE_ENTERPRISE:
		ret = C.Cell_Size_RCPRE_enterprise
	case e2sm_rc_pre_v2.CellSize_CELL_SIZE_OUTDOOR_SMALL:
		ret = C.Cell_Size_RCPRE_outdoorSmall
	case e2sm_rc_pre_v2.CellSize_CELL_SIZE_MACRO:
		ret = C.Cell_Size_RCPRE_macro
	default:
...
func decodeCellSize(cellSizeC *C.Cell_Size_RCPRE_t) e2sm_rc_pre_v2.CellSize {
