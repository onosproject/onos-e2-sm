...
func PerEncodeE2SmMhoIndicationHeader(ih *e2smmhov2.E2SmMhoIndicationHeader) ([]byte, error) {
	...
	per, err := aper.MarshalWithParams(ih, "choiceExt", e2smmhov2.MhoChoicemap, nil)
	if err != nil {
		return nil, err
	}
...
func PerDecodeE2SmMhoIndicationHeader(per []byte) (*e2smmhov2.E2SmMhoIndicationHeader, error) {

	log.Debugf("Obtained E2SM-MHO-IndicationHeader PER bytes are\n%v", hex.Dump(per))

	result := e2smmhov2.E2SmMhoIndicationHeader{}
