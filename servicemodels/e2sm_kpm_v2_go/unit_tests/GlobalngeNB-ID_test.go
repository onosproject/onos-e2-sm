...
func createGlobalNgEnbID() (*e2smkpmv2.GlobalKpmnodeId, error) {
	...
	ngeNbID.GetNgENb().GNbDuId = &e2smkpmv2.GnbDuId{
		Value: gnbDuID,
	}
...
func Test_perEncodingGlobalNgEnbID(t *testing.T) {
	...
	per, err := aper.MarshalWithParams(ngeNbID.GetNgENb().GetGlobalNgENbId(), "valueExt", e2smkpmv2.Choicemape2smKpm, nil)
	assert.NilError(t, err)
	t.Logf("GlobalNgEnbID PER\n%v", hex.Dump(per))

	result := e2smkpmv2.GlobalngeNbId{}
