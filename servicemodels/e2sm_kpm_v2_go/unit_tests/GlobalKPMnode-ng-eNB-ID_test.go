...
func createGlobalKpmnodeNgEnbID() (*e2smkpmv2.GlobalKpmnodeId, error) {
...
func Test_perEncodingGlobalKPMnodeNgEnbID(t *testing.T) {
	...
	per, err := aper.MarshalWithParams(ngeNbID.GetNgENb(), "valueExt", e2smkpmv2.Choicemape2smKpm, nil)
	assert.NilError(t, err)
	t.Logf("GlobalKPMnodeNgEnbID PER\n%v", hex.Dump(per))

	result := e2smkpmv2.GlobalKpmnodeNgEnbId{}
