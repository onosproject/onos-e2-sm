...
func Test_perEncodingTestCondExpression(t *testing.T) {
	...
	testCondExpression := e2smkpmv2.TestCondExpression_TEST_COND_EXPRESSION_PRESENT

	//aper.ChoiceMap = e2smkpmv2.Choicemape2smKpm
	per, err := aper.MarshalWithParams(testCondExpression, "valueExt,valueLB:0,valueUB:4", e2smkpmv2.Choicemape2smKpm, nil)
	assert.NilError(t, err)
	t.Logf("TestCondExpression PER\n%v", hex.Dump(per))

	var result int32
	err = aper.UnmarshalWithParams(per, &result, "valueExt,valueLB:0,valueUB:4", e2smkpmv2.Choicemape2smKpm, nil)
	assert.NilError(t, err)
